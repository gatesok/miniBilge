...
## Sprint 19 — Podcast TTS Pre-generation (Provider-Agnostic)

### Amaç

iOS native TTS yerine bulut tabanlı TTS API kullanarak doğal, ayırt edilebilir çok sesli diyalog sağlamak. Mimari provider-agnostic tasarlanacak; Google Cloud TTS ile başlanacak, ileride Azure'a geçiş yalnızca config değişikliği gerektirecek.

### Teknik Mimari

```
SpeakerName/Gender → VoiceKey (soyut) → Provider config → Gerçek ses adı
"male_1", "male_2", "female_1", "female_2"
```

**Config yapısı (appsettings.json):**
```json
{
  "TtsOptions": {
    "ActiveProvider": "Google",
    "VoiceKeys": {
      "male_1":   { "Google": "en-US-Neural2-D",  "Azure": "en-US-GuyNeural"   },
      "male_2":   { "Google": "en-US-Neural2-J",  "Azure": "en-US-TonyNeural"  },
      "female_1": { "Google": "en-US-Neural2-C",  "Azure": "en-US-JennyNeural" },
      "female_2": { "Google": "en-US-Neural2-F",  "Azure": "en-US-AriaNeural"  }
    },
    "AudioStorage": {
      "Provider": "GoogleCloudStorage",
      "BucketName": "minibilge-audio",
      "BaseUrl": "https://storage.googleapis.com/minibilge-audio/"
    }
  }
}
```

**Provider değişimi:** Yalnızca `ActiveProvider` değeri + API key. Kod değişmez.

### Faz 1 — Backend Soyutlama

| # | Görev | Öncelik |
|---|-------|---------|
| 1 | `ITtsProvider` interface — `SynthesizeAsync(text, voiceKey) → byte[]` | Yüksek |
| 2 | `ITtsAudioStorage` interface — `UploadAsync(bytes, path) → url` | Yüksek |
| 3 | `TtsProviderOptions` config class + DI kaydı (ActiveProvider'a göre inject) | Yüksek |
| 4 | `GoogleTtsProvider : ITtsProvider` implementasyonu | Yüksek |

### Faz 2 — Veritabanı & Model

| # | Görev | Öncelik |
|---|-------|---------|
| 5 | Migration: `podcast_lines.AudioUrl TEXT NULL` | Yüksek |
| 6 | Migration: `podcast_lines.VoiceKey VARCHAR(20) NULL` (örn. "male_1") | Yüksek |
| 7 | `PodcastLineDto` + API response — `AudioUrl` ve `VoiceKey` alanları | Yüksek |

### Faz 3 — Ses Üretim Servisi

| # | Görev | Öncelik |
|---|-------|---------|
| 8 | VoiceKey atama mantığı (konuşmacı sırası + gender → "male_1", "female_2") | Yüksek |
| 9 | `TtsAudioGeneratorService` — her satır → TTS → Storage → AudioUrl DB'ye yaz | Yüksek |
| 10 | `POST /api/podcast/{id}/generate-audio` endpoint (admin, idempotent) | Yüksek |
| 11 | `GoogleCloudStorageProvider : ITtsAudioStorage` implementasyonu | Yüksek |

### Faz 4 — Flutter

| # | Görev | Öncelik |
|---|-------|---------|
| 12 | `just_audio` paketi ekle (`pubspec.yaml`) | Yüksek |
| 13 | `podcast_models.dart` — `audioUrl` alanı + build_runner | Yüksek |
| 14 | `PodcastPlayerNotifier` refactor — TTS yerine AudioPlayer + URL oynatma | Yüksek |
| 15 | Fallback: `audioUrl == null` ise iOS TTS'e geri dön | Orta |
| 16 | Karaoke highlight — drop veya `Timer` bazlı satır süresi tahmini ile simülasyon | Orta |
| 17 | Ses önbellekleme stratejisi (episode açılırken tüm satırları pre-load) | Düşük |

### Faz 5 — Azure Geçişi (İleride)

| # | Görev | Öncelik |
|---|-------|---------|
| 18 | `AzureTtsProvider : ITtsProvider` implementasyonu | — |
| 19 | `AzureBlobStorageProvider : ITtsAudioStorage` implementasyonu | — |

> Bu faz aktive edilmek istendiğinde: `appsettings.json`'da `ActiveProvider: "Azure"` yap. Başka hiçbir şey değişmez.

### Görev Bağımlılık Sırası

```
1 → 2 → 3 → 4          (interface önce, impl sonra)
5 → 6 → 7              (DB önce, API sonra)
3 + 7 → 8 → 9 → 10     (servis her ikisine bağımlı)
7 → 12 → 13 → 14 → 15  (model olmadan Flutter olmaz)
14 → 16 → 17           (player tamamlanınca karaoke + cache)
```
...
|---|---|---|
| Aylık ücretsiz | 1M karakter (Neural2) | 500K karakter |
| Ücretli Neural | $16 / 1M karakter | $16 / 1M karakter |
| Ücretli HD | $100 / 1M (Studio) | **$24 / 1M** |
| Erkek ses çeşidi | 3–4 | **10+** |
| Kadın ses çeşidi | 4–5 | **10+** |

> Tahmini kullanım: 100 episode × 15 satır × 120 karakter = 180K karakter → **Her iki provider'da da ücretsiz tier içinde.**

### Kabul Kriterleri

- Podcast player her satırı ilgili ses dosyasının URL'sinden oynatır
- Farklı konuşmacılar gerçekten farklı seslerle duyulur (pitch hack gerekmez)
- `audioUrl` null olan satırlar iOS TTS ile seslendirilir (geçiş süreci)
- Provider değişikliği yalnızca `appsettings.json` güncellenerek yapılabilir
- Ses dosyaları Cloud Storage'da episode/satır bazında organize edilir

### Çıktılar

- 2 kadın + 2 erkek konuşmacıya kadar tam destek
- Doğal, akıcı, mekanik olmayan ses kalitesi
- Provider lock-in yok: Google → Azure geçişi tek config satırı
- Ses dosyaları once üretilir, sonraki oynatmalarda network maliyeti sıfır
