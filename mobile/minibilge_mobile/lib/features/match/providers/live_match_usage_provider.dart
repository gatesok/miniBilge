...
/// Kalan hak, üyelik ve ödüllü reklam uygunluğu mesaj/paywall için kullanılır.
...
final liveMatchRemainingProvider = Provider.autoDispose<int?>((ref) {
  return ref.watch(liveMatchUsageStatusProvider).maybeWhen(
        data: (status) => status?.remaining,
        orElse: () => null,
      );
});
